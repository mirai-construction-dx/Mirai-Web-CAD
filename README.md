...
![E2E](https://img.shields.io/badge/E2E-82%2F82_Passed-16A34A?style=for-the-badge&logo=playwright&logoColor=white)
...
| UI | 限定対応 | リボン、モデル/レイアウト空間タブ、タブ式右ドック、ステータスバーは動作。作図Canvasは表示寸法と描画解像度を一致させて円の縦横比とポインター座標を保ち、`ZOOM EXTENTS`/図面範囲表示は実寸Canvasへ60,000mm級の図面も余白内に収める(2026-09-25、Issue #78。高DPI描画は後続)。モバイルは固定コマンドラインがCanvasを覆う等、編集用途では未成熟(閲覧・朱書き用途を推奨) |
