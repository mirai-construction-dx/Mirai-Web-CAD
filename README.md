...
![E2E](https://img.shields.io/badge/E2E-119%2F119_Passed-16A34A?style=for-the-badge&logo=playwright&logoColor=white)
...
| 作図補助 | 限定対応 | グリッド表示・スナップ、直交モードは動作。**OSnap(2026-09-04拡張)**: 端点・中点・中心・四分点・交点を既定とし、垂線・近接点は設定ダイアログの「OSnap対象」から追加ON可能。OSnapが吸着した点は直交モードより優先する(AutoCADと同じ。吸着しなかった点だけ直交拘束)。F3(OSnap)/F7(グリッド)/F8(直交)/F9(スナップ)で切替。接線OSnap(円・円弧、既定OFF)を追加(2026-09-25)。見かけ交点・極トラッキング・追跡線は未対応 |
| コマンドライン | 限定対応 | 座標は絶対`x,y`・極`距離<角度`に加え、相対`@dx,dy`・相対極`@距離<角度`を指定可能。先頭の`@`は直前に入力した点(LASTPOINT)が基準。作図中は数値だけの入力でカーソル方向へ距離の直接入力(2026-09-25)。表示は`ZOOM E`(全体)、`ZOOM W [x1,y1 x2,y2]`(窓。点を省略するとCanvasで2回クリック)、`ZOOM 2X`/`ZOOM 0.5X`(画面中心を保った倍率)、`ZOOM P`(前の表示へ。最大20件、ホイールの連続操作は1件にまとめる)に対応し、リボンの表示タブにも窓ズーム・前画面を追加(2026-09-25)。基本作図・精密編集に加え、線分の連想Aligned/水平/垂直、円/円弧の半径/直径寸法、書式Overrideを利用可能。詳細は[連想寸法](docs/associative-dimensions.md)。角度/公差/連続寸法/名前付きStyleは後続。`HATCH`は島・境界探索・連想更新なし。`BLOCK`はchildrenを持つ簡易構造で、属性なしBlockのEXPLODEは対応、定義/参照分離・属性定義・再定義・ライブラリは後続 |
...
| Custom Domain(本番) | `https://mirai-web-cad.mirai-dx-platform.com/` | Cloudflare Tunnel経由でローカル常駐サーバーへ配信。SPAと公開デモは匿名閲覧可(匿名応答から操作者のメールアドレス等は除く)。任意図面と全更新APIはCloudflare Accessで保護。未認証アクセスはエッジ層で302(Accessログインへのリダイレクト)、アプリ層に到達した場合は401 |
