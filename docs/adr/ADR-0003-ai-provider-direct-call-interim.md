...
承認済み(Accepted、2026-09-25) — オーナーが案A(期限付きの暫定例外)と見直し期限2026-12-25を決定。移行条件が満たされるまで現行の直接呼出しを継続する(コード変更なし)。
...
2. Web-CADのsystem_id(`web-cad`、2026-09-25決定)がCoreの台帳に登録されること(Mirai-Harness-Core PR #27)。
...
5. **見直し期限: 2026-12-25**(決定日から3か月)。この日までに条件1・2が満たされない場合は、例外の継続か案Bへの切替えを改めて判断し、本ADRを更新する。
