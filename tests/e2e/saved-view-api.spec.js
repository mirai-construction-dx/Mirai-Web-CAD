...
test("same-drawing API refresh refits only when the refreshed geometry leaves the current view", async ({ page }) => {
  const moved = createDrawing({ currentRole: "drafter", entities: [
    line("layer-structure", [900000, 900000], [920000, 900000], { id: "base" }),
    circle("layer-structure", [910000, 906000], 3000, { id: "ring" })
  ] });
  let served = drawing;
  ...
  const json = (body) => ({ status: 200, contentType: "application/json", body: JSON.stringify({ ok: true, ...body }) });
  await page.route("**/api/health", (route) => route.fulfill(json({ service: "mock", auth: { mode: "demo", role: "drafter", anonymous: false }, db: { mode: "memory" } })));
  await page.route("**/api/drawings/demo", (route) => route.fulfill(json({ drawing: served })));
  await page.route("**/api/ai/status", (route) => route.fulfill(json({ enabled: false })));
  await page.goto("/");
  await expect(page.locator(".save-status")).toHaveText("サーバー同期済み");
  const box = await page.locator("#cadCanvas").boundingBox();
  ...
  await page.getByRole("button", { name: "API Health" }).click();
  await expect(page.locator(".api-status")).toContainText("同期済み");
  // 図形が現在の表示から完全に外れたため、新しい図形範囲の中心が画面中央に来る。
  // mobileではドックを開くとページがスクロールするため、Canvasの位置を取り直してから測る。
  const canvas = page.locator("#cadCanvas");
  await expect.poll(async () => {
    const current = await canvas.boundingBox();
    await canvas.hover({ position: { x: current.width / 2, y: current.height / 2 } });
    const [x, y] = (await page.locator("#coordReadout").textContent()).split(",").map((value) => Number(value.trim()));
    return Math.abs(x - 910000) < 2000 && Math.abs(y - 904500) < 2000;
  }).toBe(true);
});
